...
export default function PortableTextBlock({ value }: PortableTextBlockProps) {
  ...
  // Custom component renderers for Sanity PortableText
  const components: PortableTextComponents = {
    types: {
      // You can add custom types like image, code block, etc.
    },
    marks: {
      strong: ({ children }) => (
    ...
    block: {
      ...
      h1: ({ children }) => (
        <h1 className="text-3xl font-bold text-gray-900 mt-6 mb-2">{children}</h1>
      ),
      h2: ({ children }) => (
        <h2 className="text-2xl font-semibold text-gray-900 mt-5 mb-2">{children}</h2>
      ),
      h3: ({ children }) => (
